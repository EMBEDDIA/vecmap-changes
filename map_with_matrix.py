...
# Added saving the mapping matrices, to be used on the fly to map any word vector.
...
def main():
    ...
    parser.add_argument('--matrix', required=True, help='where to save the mapping matrices')
    ...
    if args.supervised is not None:
        parser.set_defaults(init_dictionary=args.supervised, normalize=['unit', 'center', 'unit'], whiten=False, src_reweight=0.5, trg_reweight=0.5, batch_size=1000)
    if args.semi_supervised is not None:
        parser.set_defaults(init_dictionary=args.semi_supervised, normalize=['unit', 'center', 'unit'], whiten=False, src_reweight=0.5, trg_reweight=0.5, self_learning=True, vocabulary_cutoff=20000, csls_neighborhood=10)
    if args.identical:
    ...
    if args.unsupervised or args.acl2018:
        parser.set_defaults(init_unsupervised=True, unsupervised_vocab=4000, normalize=['unit', 'center', 'unit'], whiten=False, src_reweight=0.5, trg_reweight=0.5, self_learning=True, vocabulary_cutoff=20000, csls_neighborhood=10)
    if args.aaai2018:
    ...
    #wx2, s, wz2
    if args.orthogonal or args.unconstrained:
        np.savez(args.matrix, w=w)
    else:
        np.savez(args.matrix, wx2=wx2, s=s, wz2=wz2)
